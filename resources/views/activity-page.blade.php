                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex flex-col items-center gap-1">
                        ...
                        <x-filament::badge
                            ...
                            <span class="font-bold">{{ $message['context'] }}:</span>
                            @endif
                            {{ $message['message'] }}
                        </x-filament::badge>
                        @endforeach
                    ...
                    @else
                    {{-- Chat group --}}
                    <div @class([ 'flex flex-col mb-2' , 'items-end'=> $group['is_mine'],
                        ...
                        {{-- Bubbles --}}
                        <div @class([ 'flex flex-col gap-0.5 max-w-[80%]' , 'items-end'=> $group['is_mine'],
                            'items-start' => ! $group['is_mine'],
                            ])>
                            @foreach ($group['messages'] as $message)
                            @php $isInternal = $message['is_internal']; @endphp
                            @if (! empty($message['context']))
                            <x-filament::badge size="sm" color="gray">{{ $message['context'] }}</x-filament::badge>
                            @endif
                            <div @class([ 'flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm' , 'bg-gray-200 text-gray-700 dark:bg-gray-600 dark:text-gray-200'=> $isInternal,
